defmodule MnemonicTest do
  ...
  test "Mnemonic test vectors" do
    Enum.map(mnemonic_test_vectors(), fn {lang, vectors} ->
      Enum.map(vectors, fn %{
                             entropy: entropy,
        ...
        mnemonic =
          ...
          |> Mnemonic.from_entropy(lang)

        verify_mnemonic(lang, mnemonic, expected_mnemonic)

        passphrase = vector[:passphrase] || "TREZOR"

        # We should not expect the user inputs are always normalized with right UTF8 form(NFKD)
        # so we normalize the mnemonic with wrong form(NFC) here and then do the test
        seed =
          mnemonic
          |> :unicode.characters_to_nfc_binary()
          |> Mnemonic.to_seed(passphrase, lang)
  ...
  # NOTE: Japanese test vectors are not normalized! 
  # SEE: https://github.com/bip32JP/bip32JP.github.io/commit/360c05a6439e5c461bbe5e84c7567ec38eb4ac5f
  # So we cannot compare the mnemonic in test vectors with the one we generated directly.
  defp verify_mnemonic(:japanese, mnemonic, expected_mnemonic) do
    words = String.split(mnemonic, @ideographic_space)

    expected_words =
      expected_mnemonic
      |> String.split(@ideographic_space)
      |> Enum.map(&:unicode.characters_to_nfkd_binary(&1))

    assert words == expected_words 
  end
  defp verify_mnemonic(_lang, mnemonic, expected_mnemonic) do
    assert mnemonic === expected_mnemonic 
  end

  # Test vectors from https://github.com/trezor/python-mnemonic/blob/master/vectors.json
  defp mnemonic_test_vectors do
    [
      english: [
        %{
